...
// This is the Master Matching Query we designed.
const MASTER_MATCHING_QUERY = `
  WITH UserPreferences AS (
    ...
    FROM
        \`referrals-470107.firestore_export.users_raw_latest\`
    WHERE
        JSON_EXTRACT_SCALAR(data, '$.state') = 'completed'
  ),
  PotentialMatches AS (
    SELECT
      users.user_id,
  ...
  SELECT
      ...
      pm.job_title,
      pm.job_url,
      pm.status AS user_status
  FROM PotentialMatches pm
  LEFT JOIN \`referrals-470107.matching.match_history\` hist ON pm.user_id = hist.user_id AND pm.job_id = hist.job_id
  QUALIFY ROW_NUMBER() OVER(PARTITION BY pm.user_id, pm.job_id ORDER BY hist.sent_timestamp DESC) = 1
  WHERE
      hist.sent_timestamp IS NULL OR
      (
          (pm.user_status IN ('student_position', 'no_experience_position') AND DATE_DIFF(CURRENT_DATE(), DATE(hist.sent_timestamp), DAY) > 3) OR
          (pm.user_status = 'experience_position' AND DATE_DIFF(CURRENT_DATE(), DATE(hist.sent_timestamp), DAY) > 14)
      )
`;
...
app.post('/run', async (req, res) => {
  try {
    ...
    // 1. Execute the master query in BigQuery
    ...
    for (const job of jobsToSync) {
      ...
      // Prepare BigQuery history write
      historyRows.push({
        user_id: user_id,
        job_id: job_id,
